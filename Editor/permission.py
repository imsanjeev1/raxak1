...
def initializePermission():
    permissionDict = dict() # map ruleNum to ordered dict of default args
    ...
    permissionDict['V-38449'] = OrderedDict(argsV38449)
    ...
    permissionDict['V-38457'] = OrderedDict(argsV38457)

    # V-38461
    argsV38461 = [ ('ownerRead',True), ('ownerWrite',True), ('ownerEx',False),
                   ('groupRead',True), ('groupWrite',False), ('groupEx',False),
                   ('otherRead',True), ('otherWrite',False), ('otherEx',False) ]
    permissionDict['V-38461'] = OrderedDict(argsV38461)
...
def customizePermission(data, ruleCust):
    custDefaultArgs = data.defaultArgsPermission[ruleCust] # ordered dict
    obj = Permission(custDefaultArgs)
    while True:
        userIn = raw_input("""------------
                           EDITING %s:
        ...
        try:
            ...
            if userIn == 'view args': printOrdDict(custDefaultArgs)
                ...
                data.defaultArgsPermission[ruleCust] = custDefaultArgs # necessary b/c loss of aliasing
            elif command == 'done': break
            else: assert(False)
            if command in set(['change', 'set']): obj = Permission(custDefaultArgs) # update Permissions obj
