...
//              oTable.fnUpdate('<a class="edit" href=""><i class="fa fa-fw fa-pencil"></i></a>', nRow, 4, false);
			...
			$('#editable-sample a.admin').live('click', function (e) {
                //e.preventDefault();
                var nRow = $(this).parents('tr')[0];
                var nRowID = $(this).parents('tr').attr('id');
                var admin_status = $(this).attr('admin');
                if ($(this).parents('tr').children('td').find('.admin').attr("disabled") == "disabled") {
                    e.preventDefault();
                } else {
                
                    swal({
                        text : "Are you sure to add this user to admin?",
                        type : 'warning',
                    ...
                    function(isAdd){
						if(isAdd)
						{
	                        adminPrivilege(nRowID,admin_status);
						}
                    });
