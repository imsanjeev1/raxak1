			...
			return this.each(function () {
				...
				if (o.search != false && o.optGroupSearch != false) {
					var ss = 
						...
						o.search + "<input class='small' type='text' style='margin-top: -4px;'/><a href='#'> </a>";
					...
					var	ss = o.search + "<input type='text'  style='margin-top: -4px;'/><a href='#'> </a>";
...
//						el.show().hide();
						...
						if ($(this).hasClass("AddOne")) {
							leftSel.find("option:selected").each(function(i, selected){
								...
								el.find("[value='" + $(selected).val() + "']").remove().appendTo(el).attr("selected", true);
                                                                ...
                                                                dijit.byId("applyprofile_id").setAttribute('disabled',false);//RXK #90
							});
                                                        flagIpChanges = false;
						}
							...
							if (removeFilter.is(":visible") || (searchSelect.length > 0 && searchSelect.val() != "__null__"))
								leftSel.children().each(function(i, selected){
                                    if($(selected).attr("access") === '1'){
                                        $(this).remove().appendTo(rightSel).attr("selected", true);
                                        el.find("[value='" + $(selected).val() + "']").remove().appendTo(el).attr("selected", true);
                                        dijit.byId("applyprofile_id").setAttribute('disabled',false);
                                    }
								});
                                                            ...
                                                            leftSel.children().each(function(i, selected){
                                                                        //console.log("check attribute el::"+$(this).attr("disabled"));
                                                                        if($(selected).attr("access") === '1'){
                                                                            $(this).remove().appendTo(rightSel).attr("selected", true);
                                                                            //el.find("[value='" + $(selected).val() + "']").remove().appendTo(el).attr("selected", true);
                                                                            dijit.byId("applyprofile_id").setAttribute('disabled',false);//RXK #90
                                                                ...
                                                                //console.log("check attribute ::"+$(this).attr("disabled"));
							...
							el.find('option').removeAttr("selected");
                                                        ...
                                                        dijit.byId("applyprofile_id").setAttribute('disabled',true);//RXK #90
		...
		addOption : function(options) {
			var oAddOption = {
				name: false,
				value: false,
				nickname: false,//added for rxk
				access: false,//added for rxk
				class: false,//added for rxk
				title: false,//added for rxk
				selected: false
			};
			...
			return this.each(function () {
				...
				var	strEl = "<option value='" + oAddOption.value + "' " 
                                                    + (oAddOption.selected ? "selected='selected' " : "")
                                                    + (oAddOption.nickname ? "nickname='" + oAddOption.nickname + "' " : "")
                                                    + (oAddOption.access ? "access='" + oAddOption.access + "' " : "")
                                                    + (oAddOption.class ? "class='" + oAddOption.class + "' " : "")
                                                    + (oAddOption.title ? "title='" + oAddOption.title + "' " : "")
                                                    + " >" 
